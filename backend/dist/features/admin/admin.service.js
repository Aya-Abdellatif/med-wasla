...
export class AdminService {
     ...
     */
    static async approveSpecialist(specialistId) {
        const specialist = await MedicalSpecialist.findByIdAndUpdate(specialistId, { verificationStatus: "approved" }, { new: true }).populate("userId", "name email phone address photoUrl");
        if (!specialist) {
     ...
     */
    static async rejectSpecialist(specialistId) {
        const specialist = await MedicalSpecialist.findByIdAndUpdate(specialistId, { verificationStatus: "rejected" }, { new: true }).populate("userId", "name email phone address photoUrl");
        if (!specialist) {
            throw new Error("Medical specialist not found");
        }
        return specialist;
