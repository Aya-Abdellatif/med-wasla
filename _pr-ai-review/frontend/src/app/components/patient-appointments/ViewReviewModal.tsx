    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full">
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-lg font-bold text-foreground">Your Review</h3>
        ...
        {review.comment ? (
          <p className="text-sm text-foreground bg-muted/30 p-4 rounded-xl italic leading-relaxed">"{review.comment}"</p>
        ) : (
          <p className="text-sm text-muted-foreground italic">No written comment.</p>
        )}
        <p className="text-xs text-muted-foreground mt-3">Reviewed on {formatDate(appointment.date)}</p>
        <button onClick={onClose} className="w-full mt-5 border border-border text-foreground py-2.5 rounded-xl hover:bg-muted/50 transition-colors text-sm">
          Close
        </button>
      </div>
    </div>
